...
// Create a simplified test component that focuses on the core logic
const FileSizeEstimatorCore = ({ format, quality, imageData, onSizeCalculated }) => {
  ...
  return (
    <div data-testid="file-size-estimator">
      <label>Estimated File Size</label>
      <div>
        {isCalculating ? (
          <div data-testid="spinner">Loading...</div>
        ) : (
          <span data-testid="size-value">
            {calculatedSize === '--' ? 'Generate a meme to see size' : calculatedSize}
          </span>
        )}
        <span data-testid="format-label">{formatText}</span>
      </div>
    </div>
  );
};

describe('FileSizeEstimator - Unit Tests', () => {
  const defaultProps = {
    format: 'png',
    quality: 80,
    imageData: null
  ...
  describe('Rendering', () => {
    it('should render with default props', () => {
      render(<FileSizeEstimatorCore {...defaultProps} />);
    ...
    it('should show format and quality in label', () => {
      render(<FileSizeEstimatorCore format="jpeg" quality={90} imageData={null} />);
      
      expect(screen.getByText('JPEG @ 90%')).toBeInTheDocument();
    });

    it('should not show quality for PNG format', () => {
      render(<FileSizeEstimatorCore format="png" quality={80} imageData={null} />);
  ...
  describe('Size Display', () => {
    ...
    it('should show size when image data is provided', async () => {
      render(<FileSizeEstimatorCore format="png" quality={80} imageData="data:image/png;base64,test" />);
  ...
  describe('Format Support', () => {
    it('should handle PNG format', () => {
      render(<FileSizeEstimatorCore format="png" quality={80} imageData="test" />);
      
      expect(screen.getByText('PNG')).toBeInTheDocument();
    });

    it('should handle JPEG format with quality', () => {
      render(<FileSizeEstimatorCore format="jpeg" quality={75} imageData="test" />);
      
      expect(screen.getByText('JPEG @ 75%')).toBeInTheDocument();
    });

    it('should handle WebP format with quality', () => {
      render(<FileSizeEstimatorCore format="webp" quality={90} imageData="test" />);
  ...
  describe('Size Calculation Accuracy', () => {
    ...
    it('should handle data URL size estimation accurately', async () => {
      ...
      const largerDataUrl = 'data:image/png;base64,' + 'A'.repeat(1000);
    ...
    it('should format file sizes correctly', async () => {
      const testData = 'data:image/png;base64,' + 'A'.repeat(10000);
      let result = null;
  ...
  describe('Performance and Caching', () => {
    it('should show loading state during calculation', async () => {
      ...
      render(<FileSizeEstimatorCore format="png" quality={80} imageData={testData} />);
    ...
    it('should handle rapid format/quality changes', async () => {
      ...
      const { rerender } = render(<FileSizeEstimatorCore format="png" quality={80} imageData={testData} />);
      
      // Rapidly change format and quality
      rerender(<FileSizeEstimatorCore format="jpeg" quality={90} imageData={testData} />);
      rerender(<FileSizeEstimatorCore format="webp" quality={70} imageData={testData} />);
      rerender(<FileSizeEstimatorCore format="png" quality={80} imageData={testData} />);
  ...
  describe('Accessibility', () => {
    ...
    it('should provide meaningful text for different states', async () => {
      const { rerender } = render(<FileSizeEstimatorCore format="jpeg" quality={75} imageData={null} />);
      
      expect(screen.getByText('Generate a meme to see size')).toBeInTheDocument();
      expect(screen.getByText('JPEG @ 75%')).toBeInTheDocument();
      
      rerender(<FileSizeEstimatorCore format="jpeg" quality={75} imageData="data:image/png;base64,test" />);
